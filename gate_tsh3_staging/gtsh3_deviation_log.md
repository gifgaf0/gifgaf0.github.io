...
- **H-6** (self-caught via S9-lite, post-CC) — witness-drop mechanism annotation corrected: recorded F-CONV fires at witness points were procedure noise (a*-jitter + non-deep residuals), not truncation (deep fixed-a* conv 3.5e-10 at gem8@33.03 vs recorded 1.0e-5). Drops stand as recorded (T3); D_C stays degenerate; KNOB unaffected. γ8's F-LIN component real, untouched. Successor-binding lesson: pin F-CONV operationalization (deep, fixed-a*, continuation-seeded) in future memos.
- **S9-lite closure** — CC's first-run gem8/gem4 F-CONV exclusions (conv ~4.5e-5) closed as noise-at-fragile-gate; chat counter-check `s9lite_fconv.py`: 2.27e-9 / 1.11e-9 / 5.64e-10 at the three certified kernels. Full S9 not triggered; resolution two-leg documented.
