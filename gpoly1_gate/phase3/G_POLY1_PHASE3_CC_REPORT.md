...
## 9. CC review of the re-lock-4 set (P3-A4 FROZEN; election E3-6(a); chat v5) — pre-read-#4

**Identity verification (all PASS):** frozen P3-A4 md5 `659454eda11c969ce7d6736c6f1e1063` (5,481 B), byte-exact against the RELOCK_RECORD_4 declaration (supersedes draft `7ddb6cc2`; the CC glue-class parity item incorporated with its pinned expectation). RELOCK_RECORD_4 md5 `98be3e3c0071bebad816aeca4ea96755` — matches the `relock4` field in the v5 checkpoint lock block. Chat v5 instrument pinned pre-read: md5 `2c5ca7a47042cc9929b891450f855c7e`. Read-#3 masked breach record verified `b99fa804d0bef0dc46615f2adedf86d6` (1,311 B) — matches the P3-A3 containment declaration. Chat G_HS instrument byte-identical to the dispatch embed (`fba6439a`).

**v5 stage-0 gates:** run in the CC workspace (NO-READ invocation; the sealed embed untouched): all gates green, including the three P3-A4 §2(iv) additions (identifier-glued digit; negative-lookalike positivity rejection; masked-abort mechanics). H-18(a) and (b) repairs verified present and effective: the identifier/sign guard (glue class incl. `.`), strict positivity with fall-through to inheritance, frozen-token DLM conformance, and the catch-all masked abort. Read #4 can no longer crash checkpoint-less.

**One new disclosure-level finding (CC review, demonstrated on a synthetic only — no code transferred): unit-token backtracking in the stated-path composition.** The stated-path regex is `guard + QTY + length-unit-alternation`; QTY's greedy unit group can backtrack and surrender a *suffix* of a compound unit token to the length alternation. Synthetic demonstration (committed nowhere else; structure mirrors the H-16-disclosed A-2 shape): a value carrying a compound inverse-length unit (prefix + `pc` + superscript-minus) parses as `number + 'pc'` — the prefix letter is absorbed as QTY unit-junk, the superscript satisfies `\b`, and the result is a **positive stated path**, accepted. Consequence on the sealed A-2: chat v5 will bind a stated path of 0.05 pc where CC rejects the token as inverse-dimensioned (CC-DD-2) and inherits D_ref.

**Pinned C-P3 expectations under E3-6(a), recorded pre-read:**
- **C-P3-1:** window edges (governing, A-1) — identity expected ≤1e-12 both legs. Non-governing edges flagged in advance: A2-att (CC 9.51 m at inherited D_ref vs chat ≈8.8e3 m at the 0.05 pc misbind, hex:step; both kd-valid, both non-governing) and A4-cap (CC 1.4314e4 m vs chat 1.4314e3 m — the §5(2) route divergence, unchanged by P3-A4).
- **C-P3-2 / C-P3-3:** identity expected (all arms P-2, gate P-2; census 4 rows, one per class; sealed md5 `a1d19dd9`).
- **C-P3-4:** A-1 identical (P5, identical span, B=1); A-3 now expected to **converge** (frozen-token conformance: both legs B=3.19, span `3.19`, D_ref inherited); A-2 B and value converge (B=90; spans `90%` vs `90`, trivial trailing-token difference) but the **stated-path/inheritance field diverges** (chat: stated 0.05 pc, no D_ref inherit; CC: D_ref inherited) — the single substantive S9 item, now pre-registered here rather than discovered.

**Author options on the finding:** (a) accept-and-pin — read #4 proceeds under the frozen v5; the A-2 stated-path divergence is adjudicated at C-P3-4 with this pre-read record (zero further re-locks; nothing crashes; nothing governing moves) — recommended, since a pre-registered extraction-reading divergence is exactly the evidence class the two-leg design exists to produce; or (b) amend v5 under a re-lock 5 with a left-boundary rule on the unit token (spec-level statement only; CC code not offered, per P3-A4 §4). Cosmetic nit either way: the v5 checkpoint's `read` label cites re-lock 3 while its lock block correctly carries the full chain through re-lock 4 — harmless, worth folding into any future amendment.

**Record placements in this directory (this commit):** frozen `staging_memo_G_POLY1_phase3_ADDENDUM_4.md` (`659454ed`), `G_POLY1_PHASE3_RELOCK_RECORD_4.md` (`98be3e3c`), `g_poly1_phase3_mapper_v5.py` (`2c5ca7a4`, pre-read pin), `poly1_phase3_chat_read3_X1_breach.json` (`b99fa804`, the containment record, disambiguated filename — chat-workspace name `poly1_phase3_chat.json` will be overwritten by read #4), `g_poly1_hexghs_chatleg_v3.py` + `poly1_hexghs_chat.json` (`fba6439a` / `d74916a2`, completing the two-leg G_HS closure record). CC action under E3-6(a) otherwise: none.
