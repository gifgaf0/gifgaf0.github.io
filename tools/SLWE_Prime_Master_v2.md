...
> **Note (post-v2.0; Brief 05, Brief 06 — OP-G):** the rows
> "SQT-SLWE k=32 q=911" and "SQT-SLWE k=32 q=3329" assume the
> public matrix A has full F_p-rank n = 512. Brief 05 showed that
> the **pure Singer construction** in `sqt_slwe.py` has effective
> rank 76, not 512 (column period 112; see §5.1 note above). At
> that effective rank, primal-attack lattice dimension is ≤ 112
> and the Core-SVP beta values in the security table do not apply.
> Brief 06 (Path A, `singer_a_randomized`) restores full rank 512
> by adding a uniform perturbation to the Singer base; the
...
> **Note (post-v2.0; Brief 05, Brief 06 — OP-G):** the "no linear
> dependencies" claim is established **only at k = 4**. At k = 32
> with the same pure Singer construction, F_p-rank of the flattened
> A_F drops to 76 (out of 512) and the matrix has column period 112
> (i.e. ``A_F[:, j] = A_F[:, j + 112]`` as an integer identity).
> Root cause: the Z₇ Singer cycle has order 7, so for k > 7
> columns of A_sed repeat every 7 positions, and after the 16-dim
> sedenion expansion that becomes column equality at distance
> 7·16 = 112 in A_F. See `tools/BRIEF_05_GS_PROFILE.md` for the
> measurement and `tools/BRIEF_06_OP_G.md` for the fix (randomised
> Singer construction `singer_a_randomized`, F_p-rank restored to
> 512). The k = 4 result above remains correct because 4 < 7 so no
> column repetition occurs at that scale.
...
| OP-G | Singer-orbit A matrix rank deficiency at k=32 (pure Singer F_p-rank 76, column period 112; 6.7× rank deficit). **Resolved (Brief 06)** via randomised Singer construction `singer_a_randomized` (SingerBase + δ·UniformPerturbation at δ=1, F_p-rank restored to 512). Honest caveat: at δ ≥ 1 over F_p the result is distributionally identical to a uniform random matrix, so the PSL(2,7) symmetry in A is gone; security rests on standard Module-LWE at the full rank-512 dimension. | Closed | Closed |
