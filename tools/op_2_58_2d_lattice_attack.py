...
# ---------------------------------------------------------------------------
# Fano-projected basis (b) — RATIFIED (Brief 10.6)
# ---------------------------------------------------------------------------
...
def _build_fano_augmentation_lattice(
    A_scalar: list[list[int]], b_scalar: list[int], q: int, k: int,
) -> list[list[int]]:
    """REJECTED reading (II): F_L-augmented primal lattice.

    Augments basis (a) with 14·k additional rows of form (v at block-i, 0, 0)
    for each F_L basis vector v (UNSCALED). The attacker "has the full
    problem plus the F_L hint."

    Rejected by Brief 10.6 §2 ratification: at toy k=7 q=911 β=30 across 3
    seeds, (II) was STRICTLY WEAKER than (I) under the §3.3.1 N=2.0×min_norm
    cutoff convention — BKZ finds the unscaled F_L vectors (norm ≈ 1.0) as
    the shortest rows, which hijacks the cutoff. Those F_L vectors do not
    encode the trapdoor pair, so pooled pair-recovery rate is 0/294 = 0.0000
    (vs 34/633 = 0.0537 for (I)). The hint is "too short" — it overwhelms the
    target signal under the fixed cutoff metric. See basis_b_ratification.md.
    Retained for the ratification audit record.
    """
    n = len(A_scalar)
...
def build_fano_projected_lattice(
    ...
    """RATIFIED basis (b) of §3.6 — F_L-restricted primal lattice (reading I).

    Per Brief 10.6 §2 ratification (basis_b_ratification.md): §3.6(b)'s
    "lattice basis pre-projected onto the union of the seven 8D F_L
    subspaces" is realised as **reading (I) F_L-restriction** — replace the
    e-side q·I_n rows of basis (a) with q·F_L per block, dropping the F_L⊥
    direction. The toy-scale comparison at k=7 q=911 β=30 across 3 seeds
    found (I) is the worst-case-for-defender:

      pooled pair-recovery rate (cutoff N=2.0×min_norm, baseline 1/21≈0.0476):
        (a) primal:           24/675 = 0.0356  (below baseline)
        (b)-I restriction:    34/633 = 0.0537  (above baseline — RATIFIED)
        (b)-II augmentation:   0/294 = 0.0000  (degenerate — hijacked)

    Rejected alternatives (basis_b_ratification.md §2):
      (II) F_L-augmentation — BKZ finds the unscaled F_L hint vectors as
           shortest rows, which doesn't encode the trapdoor; pooled rate 0.
           Retained internally as `_build_fano_augmentation_lattice` for the
           audit record. Inverts the brief's prediction; documented in
           basis_b_ratification.md §4.
      (III) F_L-orthogonal-quotient — rejected on prose grounds
            ("onto" F_L, not "out of"); not toy-tested.

    Construction (reading I): replace the n_eff = k·DIM rows of q·I_n on the
    e-side with 14·k rows of q·v for F_L-basis vectors v per block. The
    s-side and Kannan rows are inherited unchanged from basis (a). Result is
    rectangular: (30k+1) × (32k+1) integer matrix; lattice rank 30k+1.

    §2.58.B.1 consistency (Brief 10.6 §2.4): basis-(b) under reading (I) is
    scored by the Convention-C pair classifier (21 classes). The F_L
    restriction does NOT resolve the 42-kernel chirality (both kernels per
    pair live in the same F_L), so basis (b) remains chirality-agnostic.
    Pair-recovery is a LOWER BOUND on kernel-recovery (§2.58.B.1,
    OP-2.58.B.card): a pair-recovery null safely upper-bounds kernel leakage;
    a pair-recovery positive would warrant a 42-class follow-on classifier.

    If called with no args (orchestrator probe), runs a toy probe to confirm
    the implementation reaches a valid return.
    """
    if A_scalar is None:
    ...
    N_cols = 2 * n + 1
    rows: list[list[int]] = []
    # 14k rows: q·F_L union basis per block (restricted e-side).
    for i in range(k):
        for v in F:
            row = [0] * N_cols
            for d in range(DIM):
