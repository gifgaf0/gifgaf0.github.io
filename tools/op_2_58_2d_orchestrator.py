...
def emit_launch_command(*, prereg_path: str,
    ...
    lines = [
        ...
        "",
        "cd /path/to/gifgaf0.github.io   # set the repo root on the box",
        f"mkdir -p {run_dir}",
        "",
        "# Pre-flight gates (must all PASS):",
        "python3 tools/op_2_58_2d_bkz_driver.py",
        "",
        "# Mock validation (~3s; sanity-check the plumbing before the flip):",
        "python3 -c 'from tools.op_2_58_2d_orchestrator import run_mock; "
        "import pprint; pprint.pp(run_mock(run_dir=\"/tmp/mock_run\"))'",
        "",
        "# Binding launch (flips PRODUCTION_RUN, writes Entry 001, dispatches 42 runs):",
        ...
        "",
        f"# Watch progress: tail -f {run_dir}/audit_log.md",
        "# Resume after restart: re-run the launch command; the ledger is idempotent.",
    ]
    return "\n".join(lines)
