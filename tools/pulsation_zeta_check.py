...
⚠ CORRECTION NOTICE (two corrections after SQT-agent audits, 2026-06-06):
(1) Z1 and Z3 below are CIRCULAR — 0.0931 has NO canonical source outside the
    banked conjecture ζ = 1 − π/√12, so Z1 checks that number against its own
    rounding and Z3's "free power" only ever meets ζ := 1 − η¹ (p=1 forced by
    definition). "value = output / data selects the coupling" is WITHDRAWN.
(2) Z2's "M.CW-legal because dimensionless ⇒ not walled" is ALSO WITHDRAWN.
    Dimensionless ≠ combinatorial: ζ = 1 − π/√12 is a packing *density* (an area
    ratio needing circles, areas, π) — a *metric* quantity, which M.CW WALLS, the
    same as cos π/10 (a cosine of an angle). Both §3.4 dynamical gates are metric,
    not combinatorial, and bottom at the same metric-instantiation wall; there is
    no "legal vs walled" asymmetry.
The ONLY solid positive content is the verified pulsation (0.0948, Regge cascade)
≠ ζ (0.0931) finding — "pulsation = ζ" is the leading term, not an identity.
See reports/SQT_2.52_PULSATION_ZETA_CHECK.md §0′ and §0.
"""
...
def main():
    ...
    print("=" * 78)
    print("  ⚠ CORRECTION NOTICE (SQT-agent audits): Z1/Z3 are CIRCULAR (check the")
    print("    conjecture ζ=1−π/√12 against itself) ⇒ 'value=output' WITHDRAWN. AND")
    print("    Z2's 'M.CW-legal because dimensionless' is WITHDRAWN too: ζ is a *metric*")
    print("    packing ratio (areas, π), not combinatorial — M.CW walls it, same as")
    print("    cos π/10. No legal-vs-walled asymmetry. Only solid result: pulsation")
    print("    (0.0948) ≠ ζ (0.0931). See report §0′/§0.")
    print()
